...
if (args$date == 'max'){
  today <- as.Date(as.character(max(d$date)), format='%Y%m%d')
...
dquantile <- dquantile[dquantile$dado == args$type & dquantile$epiyearweek == max(d$epiyearweek), c('UF', 'epiyearweek', 'delayweeks')]
