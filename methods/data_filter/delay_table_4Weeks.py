...
def readtable(fname):
    tgt_cols = ['SG_UF_NOT', 'DT_NOTIFIC_epiyear', 'DT_NOTIFIC_epiweek', 'Notific2Digita_DelayWeeks']
    df = pd.read_csv(fname, low_memory=False)[tgt_cols].rename( columns={'SG_UF_NOT': 'UF',
                                                                         'DT_NOTIFIC_epiyear': 'epiyear',
                                                                         'DT_NOTIFIC_epiweek': 'epiweek',
                                                                         'Notific2Digita_DelayWeeks': 'DelayWeeks'})
...
def cumhistfull(x):
    ...
    dfcumhist = pd.DataFrame([[a,b] for a,b in zip(range(0,min(xlim,9)), h[0:min(xlim,9)])], columns=['Weeks',
                                                                                                      'Cumulative'])
    if xlim > 9:
        dfcumhist = dfcumhist.append(pd.DataFrame([{'Weeks': '> 8', 'Cumulative': 100}]))
    return(dfcumhist)
...
def createtable(df=pd.DataFrame(), testmonth=None, testyear=None):
    ...
    # Filter data keeping only Notifications within desired window:
...
def histograms(df):
    df4w = df[['UF', 'epiyear', 'epiweek', 'd0', 'd1','d2','d3','d4','d5']]
    cols=['UF', 'd0', 'd1','d2','d3','d4','d5']
