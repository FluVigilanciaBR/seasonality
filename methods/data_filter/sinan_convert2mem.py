...
def readtable(fname, sep=','):
    ...
    df = pd.concat([df, dftmp], ignore_index=True, sort=True)
    ...
    dffull_reg = pd.concat([dffull_reg, dffull_reg_ofi], ignore_index=True, sort=True)
    dffull_reg = pd.concat([dffull_reg, dfBR], ignore_index=True, sort=True)

    dffull = pd.concat([dffull.drop(['Região', 'Região oficial'], axis=1),
                        dffull_reg],
                       ignore_index=True,
                       sort=True)
