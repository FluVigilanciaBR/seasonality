...
def region4mem(dfin):
    ...
    dfreg = pd.read_csv('regioesclimaticas.csv')
    ...
    df = dftmp.sort(columns=['Região', 'isoweek']).reset_index().drop('index', axis=1)
    dfinc = dftmpinc.sort(columns=['Região', 'isoweek']).reset_index().drop('index', axis=1)
...
def main(fname, sep=','):
    ...
    # Region data:
    df_region, df_region_inc = uf4mem(df)


    # Write output to file:
    fnameout = '.'.join(fname.split('.')[:-1]) + '4mem-regiao-incidence.csv'
    dfinc.rename(columns={'Região': 'UF'}).to_csv(fnameout, index=False)
    fnameout = '.'.join(fname.split('.')[:-1]) + '4mem-regiao.csv'
    df.rename(columns={'Região': 'UF'}).to_csv(fnameout, index=False)
